...
select LAST_INSERT_ID() into i_mqtt_server_id;
...
,'InternalLogin'
,'1nternalPassW0rd'
,'481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134'
,i_mqtt_server_id
);
...
CREATE DEFINER=`kalistrat`@`localhost` PROCEDURE `addPackageSensor`(
	IN `eSnsPref` VARCHAR(10),
	IN `eChildUID` VARCHAR(50),
	IN `eUserId` INT,
	IN `eUserLog` VARCHAR(50),
	IN `eMqttServerId` INT,
	IN `eControlLog` VARCHAR(50),
	IN `eControlPass` VARCHAR(50),
	IN `eNextLeafId` INT,
	IN `eParentTreeId` INT,
	IN `eSyncTime` INT,
	OUT `oTaskId` INT,
	OUT `oSnsToTopic` VARCHAR(150)
)
BEGIN
...
-- Дамп данных таблицы things.dropped_user_devices: ~3 rows (приблизительно)
DELETE FROM `dropped_user_devices`;
/*!40000 ALTER TABLE `dropped_user_devices` DISABLE KEYS */;
INSERT INTO `dropped_user_devices` (`droped_devices_id`, `parent_uid`, `uid`, `to_server_topic`, `from_server_topic`, `timezone`) VALUES
	(3, 'BRI-N6EZ7ZKS0DHB', 'SEN-ODFVZ2K2RV6Q', '/BRI-N6EZ7ZKS0DHB/to', '/BRI-N6EZ7ZKS0DHB/timesync', 'UTC+3'),
	(4, 'BRI-U1AUFRTJ8ZTD', 'SEN-CIJ5H1ER2U3R', '/BRI-U1AUFRTJ8ZTD/to', '/BRI-U1AUFRTJ8ZTD/timesync', 'UTC+3'),
	(6, 'BRI-N6EZ7ZKS0DHB', 'SEN-QJ99D9R2TWDN', '/BRI-N6EZ7ZKS0DHB/to', '/BRI-N6EZ7ZKS0DHB/timesync', 'UTC+3'),
	(7, 'BRI-N6EZ7ZKS0DHB', 'BRI-N6EZ7ZKS0DHB', '/BRI-N6EZ7ZKS0DHB/to', '/BRI-N6EZ7ZKS0DHB/timesync', 'UTC+3');
...
from user_devices_tree udt
join users u on u.user_id=udt.user_id
join user_devices_tree pudt on udt.parent_leaf_id=pudt.leaf_id and udt.user_id=pudt.user_id
where udt.leaf_id = iLeafId
and u.user_log = iUserLogin;

if (i_parent_uid = iUserLogin) then
...
-- Дамп структуры для процедура things.getMqttConnetionArgsDropUID
DELIMITER //
CREATE DEFINER=`kalistrat`@`localhost` PROCEDURE `getMqttConnetionArgsDropUID`(
	IN `iUID` VARCHAR(50)
,
	IN `iUserLogin` VARCHAR(150)
,
...
join mqtt_servers ms on ms.server_id=udt.mqtt_server_id
join users u on u.user_id=udt.user_id
where u.user_log=iUserLogin
and udt.parent_leaf_id is null;

select dud.from_server_topic
,dud.uid
,dud.timezone
into oTopicName
,oUID
,oTimeZone
from dropped_user_devices dud
where dud.uid=iUID;
...
declare i_parent_uid varchar(150);
declare i_uid varchar(150);
declare i_user_login varchar(150);
...
if (i_parent_uid = i_user_login) then
...
-- Дамп структуры для процедура things.getSensorTopicUID
DELIMITER //
CREATE DEFINER=`kalistrat`@`localhost` PROCEDURE `getSensorTopicUID`(
	IN `UID` VARCHAR(50),
	OUT `oTopic` VARCHAR(150),
	OUT `oUserName` VARCHAR(150)
)
BEGIN
...
from user_devices_tree udt
join users u on u.user_id=udt.user_id
join user_device ud on ud.user_device_id=udt.user_device_id
where udt.uid = UID;
...
INSERT INTO `mqtt_servers` (`server_id`, `server_ip`, `server_port`, `is_busy`, `name`, `server_type`, `vserver_ip`, `user_id`) VALUES
	...
	(16, 'tcp://0.0.0.0:9009', 9009, 0, 'LOCALHOST', 'regular', 'tcp://snslog.ru:9009', 8),
	(17, 'ssl://0.0.0.0:9010', 9010, 0, 'LOCALHOST', 'ssl', 'ssl://snslog.ru:9010', 8),
	(18, 'tcp://0.0.0.0:9011', 9011, 0, 'LOCALHOST', 'regular', 'tcp://snslog.ru:9011', 9),
...
DELIMITER //
CREATE DEFINER=`kalistrat`@`localhost` PROCEDURE `pSaveDroppedDevice`(
	IN `oTreeId` INT
...
declare i_parent_pref varchar(20);
declare i_child_pref varchar(20);
declare i_device_topic_name  varchar(100);
declare i_child_leaf_name varchar(150);
declare i_user_id int;
declare i_mqtt_server_id int;
declare i_control_log varchar(100);
declare i_control_pass varchar(100);
declare i_user_device_id int;
declare i_host_name varchar(150);
...
set i_device_topic_name = concat('/',eChildUID);

if (i_child_pref = 'BRI') then
...
DELIMITER //
CREATE DEFINER=`kalistrat`@`localhost` PROCEDURE `updateMeteoPackage`(
...
set oPackLog = i_control_log;
set oPackPass = i_control_pass;
set oPackHost = i_host_name;
set oPackFromTopic = i_from_topic;
set oPackToTopic = i_to_topic;
...
INSERT INTO `users` (`user_id`, `user_log`, `user_pass`, `user_last_activity`, `user_mail`, `user_phone`, `first_name`, `second_name`, `middle_name`, `birth_date`, `subject_type`, `subject_name`, `subject_address`, `subject_inn`, `subject_kpp`, `post_index`) VALUES
	...
	(2, 'Oleg', '2c624232cdd221771294dfbb310aca000a0df6ac8b66b696d90ef06fdefb64a3', '2017-01-12 18:02:48', '765567@mail.ru', '34634634634', 'Oleg', 'Antipov', NULL, '1987-07-13', 'физическое лицо', NULL, NULL, NULL, NULL, NULL),
	...
	(8, 'semenovna', '4c4de41965012a94d6fcca5abbb792139e5f88f9149af99571be04d6b1afdcee', NULL, 'n7.semenov@gmail.com', '159951', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
...
INSERT INTO `user_device` (`user_device_id`, `user_id`, `device_user_name`, `user_device_mode`, `user_device_measure_period`, `user_device_date_from`, `action_type_id`, `device_units`, `mqtt_topic_write`, `mqtt_topic_read`, `mqqt_server_id`, `unit_id`, `factor_id`, `description`, `device_log`, `device_pass`, `measure_data_type`) VALUES
	...
	(21, 1, 'SEN-MRCBPZYVNKLT', NULL, 'не задано', '2018-06-08 23:25:49', 1, 'Ед', '/SEN-MRCBPZYVNKLT', '21', 4, 96, 64, 'SEN-MRCBPZYVNKLT', 'kr3A3r', 'WSlg1Lt', 'число'),
	(28, 1, 'Датчик температуры', NULL, 'не задано', '2018-06-12 17:05:39', 1, 'Ед', '/MET-QNFAAC0KISU2-TEM', '28', 4, 96, 64, 'Датчик температуры', 'k4GdBY', 'ylZ2h7l', 'число'),
	(29, 1, 'Датчик влажности', NULL, 'не задано', '2018-06-12 17:05:40', 1, 'Ед', '/MET-QNFAAC0KISU2-HUM', '29', 4, 96, 64, 'Датчик влажности', 'k4GdBY', 'ylZ2h7l', 'число'),
	(30, 1, 'Датчик освещённости', NULL, 'не задано', '2018-06-12 17:05:40', 1, 'Ед', '/MET-QNFAAC0KISU2-LGHT', '30', 4, 96, 64, 'Датчик освещённости', 'k4GdBY', 'ylZ2h7l', 'число');
...
INSERT INTO `user_devices_tree` (`user_devices_tree_id`, `leaf_id`, `parent_leaf_id`, `user_device_id`, `leaf_name`, `user_id`, `timezone_id`, `mqtt_server_id`, `time_topic`, `sync_interval`, `control_log`, `control_pass`, `control_pass_sha`, `leaf_type`, `uid`, `to_server_topic`) VALUES
	(1, 1, NULL, NULL, 'Устройства', 1, NULL, 4, NULL, NULL, 'InternalLogin', '1nternalPassW0rd', '481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134', 'FOLDER', 'k', NULL),
	(2, 1, NULL, NULL, 'Устройства', 2, NULL, 6, NULL, NULL, 'InternalLogin', '1nternalPassW0rd', '481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134', 'FOLDER', 'Oleg', NULL),
	(3, 1, NULL, NULL, 'Устройства', 5, NULL, 9, NULL, NULL, 'InternalLogin', '1nternalPassW0rd', '481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134', 'FOLDER', 'qweqwe123', NULL),
	(5, 1, NULL, NULL, 'Устройства', 7, NULL, 11, NULL, NULL, 'InternalLogin', '1nternalPassW0rd', '481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134', 'FOLDER', 'kalistrat', NULL),
	...
	(29, 1, NULL, NULL, 'Устройства', 8, NULL, 17, NULL, NULL, 'InternalLogin', '1nternalPassW0rd', '481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134', 'FOLDER', 'semenovna', NULL),
	...
	(32, 1, NULL, NULL, 'Устройства', 9, NULL, 19, NULL, NULL, 'InternalLogin', '1nternalPassW0rd', '481f495cfee4c12d871b2379510e9a1a5c88ae20ebe61a7678ba22c50dc9e134', 'FOLDER', 'akmakmakm', NULL),
	(42, 2, 1, NULL, 'BRI-FMLD7DHZG2VG', 1, 17, 4, '/BRI-FMLD7DHZG2VG/timesync', 1, 'kCtX4s', 'VzcRKbu', '2be184527c257d6b30d795576b2986dbf5b06f33e4830efa63db03dc38997b0c', 'FOLDER', 'BRI-FMLD7DHZG2VG', '/BRI-FMLD7DHZG2VG/to'),
	(43, 3, 2, 16, 'SEN-CS3VEK79WZFD', 1, 17, 4, '/SEN-CS3VEK79WZFD/timesync', 2, 'kCtX4s', 'VzcRKbu', '2be184527c257d6b30d795576b2986dbf5b06f33e4830efa63db03dc38997b0c', 'SENSOR', 'SEN-CS3VEK79WZFD', '/SEN-CS3VEK79WZFD/to'),
	(44, 4, 1, NULL, 'MET-23JWEVCSXRB3', 1, 17, 4, '/MET-23JWEVCSXRB3/timesync', 1, 'kZQqvn', 'K91127p', '20c6d7ff5af98b1c8723e9d56ec8b1d6225ad4e0e39a678db9954763f896e161', 'FOLDER', 'MET-23JWEVCSXRB3', '/MET-23JWEVCSXRB3/to'),
	...
	(51, 11, 1, NULL, 'BRI-C4CJZMXKDJDK', 1, 17, 4, '/BRI-C4CJZMXKDJDK/timesync', 1, 'kr3A3r', 'WSlg1Lt', 'bca63b123b679d590ec492c9c685c8c2cee7c4397fe7ff868c24346037103a85', 'FOLDER', 'BRI-C4CJZMXKDJDK', '/BRI-C4CJZMXKDJDK/to'),
	(52, 12, 11, 21, 'SEN-MRCBPZYVNKLT', 1, 17, 4, '/SEN-MRCBPZYVNKLT/timesync', 1, 'kr3A3r', 'WSlg1Lt', 'bca63b123b679d590ec492c9c685c8c2cee7c4397fe7ff868c24346037103a85', 'SENSOR', 'SEN-MRCBPZYVNKLT', '/SEN-MRCBPZYVNKLT/to'),
	(62, 13, 1, NULL, 'MET-QNFAAC0KISU2', 1, 17, 4, '/MET-QNFAAC0KISU2/timesync', 1, 'k4GdBY', 'ylZ2h7l', '8b03706578af258d5a63efbd5bc61226e269255737a17ef5bf1a3dde913ecaa4', 'FOLDER', 'MET-QNFAAC0KISU2', '/MET-QNFAAC0KISU2/to'),
	(63, 14, 13, 28, 'Датчик температуры', 1, 17, 4, '/MET-QNFAAC0KISU2/timesync', 1, 'k4GdBY', 'ylZ2h7l', '8b03706578af258d5a63efbd5bc61226e269255737a17ef5bf1a3dde913ecaa4', 'SENSOR', 'MET-QNFAAC0KISU2-TEM', '/MET-QNFAAC0KISU2/to'),
	(64, 15, 13, 29, 'Датчик влажности', 1, 17, 4, '/MET-QNFAAC0KISU2/timesync', 1, 'k4GdBY', 'ylZ2h7l', '8b03706578af258d5a63efbd5bc61226e269255737a17ef5bf1a3dde913ecaa4', 'SENSOR', 'MET-QNFAAC0KISU2-HUM', '/MET-QNFAAC0KISU2/to'),
	(65, 16, 13, 30, 'Датчик освещённости', 1, 17, 4, '/MET-QNFAAC0KISU2/timesync', 1, 'k4GdBY', 'ylZ2h7l', '8b03706578af258d5a63efbd5bc61226e269255737a17ef5bf1a3dde913ecaa4', 'SENSOR', 'MET-QNFAAC0KISU2-LGHT', '/MET-QNFAAC0KISU2/to');
